...
public class EmailDuplicateCheck3Controller extends AbstractController {
...
String method =	request.getMethod(); // "GET" 또는 "POST"
		...
		if("POST".equalsIgnoreCase(method)) {
			...
			boolean isExists = mdao.emailDuplicateCheck3(email); 
			// 자신이 사용중인 이메일인지 아닌지 알아오는 메소드
			
			
			if(!isExists) { // 자신이 변경하고자 하는 이메일이 자신이 사용중이지 않는 새로운 이메일 이라면(false)
				isExists = mdao.emailDuplicateCheck(email);//자신이 변경하고자하는 이메일이 tbl_member 테이블에서 사용중인지 아닌지 여부 알아오기
