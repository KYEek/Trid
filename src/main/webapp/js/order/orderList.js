...
/*<div class="order_list_item">
          <div class="order_itme_header">
            ...
            <div class="order_itme_price_detail">
              <span>¥가격</span> <a href="">주문 보기</a>
            </div>
          </div>
          <div class="order_item_images">
            <img src="images/basket/00155055500-p.jpg" /><img
              src="images/basket/00155055500-p.jpg"
            />
            <img src="images/basket/00155055500-p.jpg" />
          </div>
        </div>*/
// const orderListStr = `${requestScope.orderList}`;
// orderListStr.replace("\\", "\\\\");
document.addEventListener("DOMContentLoaded", function () {
  ...
  orderList.forEach((item) => {
    ...
    if (FK_ORDER_NO == "") {
      ...
      html += `<div class="order_list_item">
          <div class="order_itme_header">
            ...
            <div class="order_itme_price_detail">
              <span>₩${ORDER_TOTAL_PRICE}</span> <a href="">주문 보기</a>
            </div>
          </div>
          <div class="order_item_images">
            <img src="${PRODUCT_IMAGE_NAME}" />`;
    } //end of if(FK_ORDER_NO == "")
    //새로운 주문번호가 들어왔을 때
    else if (FK_ORDER_NO != item.FK_ORDER_NO) {
      FK_ORDER_NO = item.FK_ORDER_NO;
      ORDER_DATE = item.ORDER_DATE;
      ORDER_STATUS = item.ORDER_STATUS;
      ORDER_TOTAL_PRICE = item.ORDER_TOTAL_PRICE;
    ...
    //기존 주문번호와 같은 주문번호가 들어왔을 때
      ...
      //이미지가 다를 때만 추가
      if (PRODUCT_IMAGE_NAME != item.PRODUCT_IMAGE_NAME) {
        PRODUCT_IMAGE_NAME = item.PRODUCT_IMAGE_NAME;
        html += `<img src="${PRODUCT_IMAGE_NAME}" />`;
      }
    } //end of else
    // } //end of for
  }); //end of forEach
